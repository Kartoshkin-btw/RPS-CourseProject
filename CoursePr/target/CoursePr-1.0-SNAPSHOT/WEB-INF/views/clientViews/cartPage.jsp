    ...
    <form:form action="saveOrder" method="post">
        <table border="1">
            ...
            <c:forEach var="item" items="${sessionScope.cart}" varStatus="status">
                <tr>
                    <td><a class = "choose" href="removeLine?clothesID=${item.clothes.id}&templateID=${item.print.template.id}&imageID=${item.print.image.id}&size=${item.size}">Remove line</a></td>
                    ...
                    <td><a class = "choose" href="incQuantity?clothesID=${item.clothes.id}&templateID=${item.print.template.id}&imageID=${item.print.image.id}&size=${item.size}">+</a></td>
                    <td><a class = "choose" href="decQuantity?clothesID=${item.clothes.id}&templateID=${item.print.template.id}&imageID=${item.print.image.id}&size=${item.size}">-</a></td>
                </tr>
            </c:forEach>
        </table>
