...
public class OrderItemDAO {
    ...
    public void saveOrUpdate (OrderItem oi){
        if(oi.getId() > 0){
            String sql = "UPDATE OrderItem SET orderID=?, clothesID=?, printID=?, size=?, quantity=? WHERE id='"+oi.getId()+"';";
            jdbcTemplate.update(sql, oi.getOrderID(), oi.getClothes().getId(), oi.getPrint().getId(), oi.getSize(), oi.getQuantity());
        } else {
            String sql = "INSERT INTO OrderItem (orderID, clothesID, printID, size, quantity) VALUES (?,?,?,?,?)";
    ...
    public List<OrderItem> getContentByOrderID (final Integer id){
        ...
        RowMapper<OrderItem> rowMapper = new RowMapper<OrderItem>() {
            ...
            public OrderItem mapRow(ResultSet resultSet, int i) throws SQLException {
                ...
                return new OrderItem(id, orderID, clothesDAO.getClothesById(clothesID), printDAO.getPrintById(printID), size, quantity);
            }
        };
