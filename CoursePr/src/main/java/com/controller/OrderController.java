...
public class OrderController {
    ...
    public ModelAndView confirmPayment(@ModelAttribute Order order) {
        List<OrderItem> itemList = orderItemDAO.getContentByOrderID(order.getId());
        for (OrderItem orderItem: itemList){
            double lineSum = (orderItem.getClothes().getPrice() + orderItem.getPrint().getTemplate().getPrice() * orderItem.getQuantity());
            if(!orderItem.getPrint().getImage().getAuthor().equals(""))  {
                Users user = usersDAO.findByUserName(orderItem.getPrint().getImage().getAuthor());
                double newBalance = user.getBalance() + lineSum * 0.1;
    ...
    public ModelAndView saveOrder(HttpServletRequest request, HttpSession session) {
        ...
        for (Item item:cart) {
            OrderItem orderItem = new OrderItem(lastID, clothesDAO.getClothesById(item.getClothes().getId()), printDAO.getPrintById(item.getPrint().getId()), item.getSize(), item.getQuantity());
