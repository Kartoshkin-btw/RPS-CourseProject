...
public class CartController {
    ...
    @RequestMapping(value = "/addCombinationToCart", method = RequestMethod.POST)
    public String addToCart(ModelAndView model, HttpServletRequest request, HttpSession session){
        ...
        if(session.getAttribute("cart") == null){
            ...
            cart.add(new Item(combination.getClothes(), combination.getPrint(), 1, size));
            session.setAttribute("cart", cart);
        } else{
            List<Item> cart = (List<Item>)session.getAttribute("cart");
            int index = exists(combination.getClothes().getId(), combination.getPrint().getTemplate().getId(),
                    combination.getPrint().getImage().getId(), size, session);
            if (index == -1){
                cart.add(new Item(combination.getClothes(), combination.getPrint(), 1, size));
            } else {
    ...
    //Изменение размеров
    @RequestMapping(value = "/applyClothes", method = RequestMethod.POST)
    public ModelAndView saveCombination(HttpServletRequest request, ModelAndView model, RedirectAttributes redirectAttributes) {
        String size = request.getParameter("size");
        int clothesID = Integer.parseInt(request.getParameter("clothesID"));
        redirectAttributes.addFlashAttribute("size", size);
        redirectAttributes.addFlashAttribute("clothesID", clothesID);
        model.setViewName("redirect:/listImages");
        return model;
    }
    ...
    public ModelAndView chooseTemplate(ModelAndView model, HttpServletRequest request, HttpSession session){
        ...
        model.addObject("clothesID", clothesID);
    ...
    public String addCustomCombination(ModelAndView model, HttpServletRequest request, HttpSession session){
        ...
        if(session.getAttribute("cart") == null){
            List<Item> cart = new ArrayList<Item>();
            cart.add(new Item(clothes, savedPrint, 1, size));
            session.setAttribute("cart", cart);
        } else {
            List<Item> cart = (List<Item>)session.getAttribute("cart");
            int index = exists(clothesID, templateID, imageID, size, session);
            if (index == -1){
                cart.add(new Item(clothes, savedPrint, 1, size));
            } else {
                int quantity = cart.get(index).getQuantity() + 1;
    ...
    static private int exists(int clothesID, int templateID, int imageID, String size, HttpSession session){
        List<Item> cart = (List<Item>)session.getAttribute("cart");
        for(int i=0; i<cart.size(); i++)
            if((cart.get(i).getClothes().getId() == clothesID) & (cart.get(i).getPrint().getImage().getId() == imageID)
                    & (cart.get(i).getPrint().getTemplate().getId() == templateID) & (cart.get(i).getSize().equals(size))) return i;
        return -1;
        }
    }
